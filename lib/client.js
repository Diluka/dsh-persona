...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** @typedef {{ preset: PresetId, shared: string }} PersonaSettings */
    ...
    const zh = /** @type {Record<string, string>} */ ({
      nav: "沟通风格",
      title: "助手沟通风格",
      intro: "选择内置风格并查看完整只读提示词；共通部分放在页面最后供编辑。插件运行状态由 DSH 插件管理器控制。",
      loading: "正在读取设置...",
    ...
    const en = /** @type {Record<string, string>} */ ({
      ...
      intro: "Choose a built-in style and review its complete read-only prompt; edit the shared prompt at the end. Use the DSH Plugins manager to control whether this plugin runs.",
    ...
    function sameSettings(a, b) {
      return a.preset === b.preset && a.shared === b.shared;
    }

    /** @param {unknown} user @returns {string[]} */
    function overrideFields(user) {
      return isRecord(user) ? Object.keys(user).filter((key) => ["preset", "shared"].includes(key)) : [];
    }
    ...
    function PersonaSettingsSection(props) {
      ...
      const fallback = /** @type {PersonaSettings} */ ({ preset: "", shared: "" });
