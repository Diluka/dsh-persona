...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** @typedef {import("@deepseek-ai/dsh-client-ui-settings/client").ConfigForm<PersonaSettings>} SettingsScope */
    /** @typedef {import("@deepseek-ai/dsh-client-ui-settings/client").ConfigFormSnapshot<PersonaSettings>} ScopeSnapshot */
    /** @typedef {import("@deepseek-ai/dsh-client-ui-settings/client").SettingsMirrorSnapshot} MirrorSnapshot */
    /** @typedef {import("@deepseek-ai/dsh-client-ui-settings/client").SettingsDescribeFace} SettingsDescribeFace */
    ...
    /** @typedef {{ slots: { inject(name: string, factory: () => () => void): () => void, register(options: Record<string, unknown>, component: unknown): () => void }, locale: { register(ns: string, messages: { zh: Record<string, string>, en: Record<string, string> }): () => void, bind(ns: string): Translate }, configForms: { get(entryId: string): SettingsScope, describe(): SettingsDescribeFace }, effect(register: () => (() => void), label?: string): void }} ClientContext */

    const SETTINGS_NAMESPACE = "dsh-persona";
    const SETTINGS_ENTRY_ID = "include:dsh-persona";
    ...
    const css = `
      ...
      .dshPersonaPluginHeader:focus-visible {
      ...
      .dshPersonaPluginRepo:focus-visible {
      ...
      .dshPersonaSwitch[aria-checked="true"] {
      ...
      .dshPersonaSwitch:disabled {
        cursor:not-allowed;
        opacity:.55;
      }

      .dshPersonaSwitch:focus-visible {
        outline:2px solid var(--dsw-alias-brand-primary);
        outline-offset:2px;
      }

      .dshPersonaSwitchThumb {
        width:16px;
        height:16px;
        border-radius:50%;
      ...
      .dshPersonaSwitch[aria-checked="true"] .dshPersonaSwitchThumb {
    ...
    function catalogFromMirror(snapshot) {
      ...
      const namespace = namespaces.find((entry) => entry.ns === SETTINGS_ENTRY_ID);
    ...
    async function writeInheritedSettingField(scope, snapshot, field, value, inherited) {
      const accepted = value === inherited && isOverridden(snapshot, field)
        ? await scope.unset(field)
        : await scope.set(field, value);
      if (!accepted) throw new Error("The configuration update was not accepted");
    }
    ...
    /** @typedef {{ checked: boolean, label: string, disabled: boolean, onChange: (checked: boolean) => void }} SwitchControlProps */
    /** @param {SwitchControlProps} props @returns {import("react").ReactElement} */
    function SwitchControl(props) {
      return h("button", {
        type: "button",
        className: "dshPersonaSwitch",
        role: "switch",
        "aria-checked": props.checked,
        "aria-label": props.label,
        disabled: props.disabled,
        onClick: () => props.onChange(!props.checked)
      }, h("span", { className: "dshPersonaSwitchThumb", "aria-hidden": true }));
    }
    ...
    function PluginConfigCard(props) {
      ...
      return h("div", { className: open ? "dshPersonaPluginCard dshPersonaPluginCardOpen" : "dshPersonaPluginCard" },
        h("button", {
          type: "button",
          className: "dshPersonaPluginHeader",
          "aria-expanded": open,
          ...
          h("span", { className: open ? "dshPersonaPluginChevron dshPersonaPluginChevronOpen" : "dshPersonaPluginChevron", "aria-hidden": true }, "⌄")
        ),
        open ? h("div", { className: "dshPersonaPluginBody" },
          ...
          h("div", { className: "dshPersonaPluginRow" },
            ...
            h(SwitchControl, { checked: effectiveDraftEnabled, label: props.t("enabled"), disabled, onChange: (checked) => setDraftEnabled(checked) })
          ),
    ...
    function PersonaSettingsSection(props) {
      ...
      if (snapshot.status === "loading" || catalogSnapshot.status === "loading" || catalogSnapshot.status === "idle") {
    ...
    function apply(ctx) {
      ...
      const scope = ctx.configForms.get(SETTINGS_ENTRY_ID);
      const catalogSource = ctx.configForms.describe();
      ...
      ctx.effect(() => ctx.slots.inject("settings.plugins.tab", () => ctx.slots.register({
        ...
        order: 30,
        label: () => t("pluginTitle")
      }, () => h(PluginConfigCard, { t, scope }))), "dsh-persona: plugin settings tab");
      ctx.effect(() => {
        /** @type {undefined | (() => void)} */
        let disposeSection;
