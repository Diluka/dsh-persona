...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** @param {SettingsScope} scope @param {ScopeSnapshot} snapshot @param {PersonaCatalog} catalog @param {keyof PersonaSettings} field @param {PersonaSettings[keyof PersonaSettings]} value */
    async function writeSettingField(scope, snapshot, catalog, field, value) {
      const inherited = normalize(snapshot.base, catalog);
      if (value === inherited[field] && isOverridden(snapshot, field)) await scope.unset(field);
    ...
    function PluginConfigCard(props) {
      ...
      async function save() {
        ...
        setFailed(false);
        try {
          if (effectiveDraftEnabled === inheritedEnabled && isOverridden(snapshot, "enabled")) await props.scope.unset("enabled");
          else await props.scope.set("enabled", effectiveDraftEnabled);
        } catch (_err) {
          setFailed(true);
        } finally {
          setSaving(false);
    ...
    function PersonaSettingsSection(props) {
      ...
      const catalogSnapshot = React.useSyncExternalStore(
        props.catalogSource.subscribe.bind(props.catalogSource),
        props.catalogSource.getSnapshot.bind(props.catalogSource),
        props.catalogSource.getSnapshot.bind(props.catalogSource)
      );
      React.useEffect(() => {
      ...
      async function save() {
        ...
        try {
          ...
          for (const field of fields) {
            ...
            await writeSettingField(props.scope, snapshot, catalog, field, draft[field]);
          }
        } catch (err) {
